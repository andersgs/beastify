...
from nexus.Genes import Genes
from nexus.Collection import Collection
from nexus.tests.Test_Nexus import TestBeastify
import global_def
...
@click.version_option( version = global_def.VERSION, message = "beastify v{}".format( global_def.VERSION ) )
...
def beastify(reference, out, info, inc_ref, aln_file, aln_file_format, subsample, subsample_seed, parts, mask):
    ...
    collection.make_nexus(out, genes, subsample = subsample, subsample_seed = subsample_seed, partitions = parts, mask = mask)
    return
